...
const EditProductForm = () => {
  ...
  //fetch the edited product
  const productData = useQuery(
    ["productData" + id, id],
    async () =>
      await axios.get(
        `${import.meta.env.VITE_REACT_APP_BACKEND_URL}product/find/${id}`,
        {
          headers,
        }
      ),
  ...
  const initialValues: IProductFormProps = {
    category: productData?.data?.data?.data?.category?._id,
    name: productData?.data?.data?.data?.name,
    nameAm: productData?.data?.data?.data?.nameAm,
    description: productData?.data?.data?.data?.description,
    descriptionAm: productData?.data?.data?.data?.descriptionAm,
    image: [],
    price: productData?.data?.data?.data?.price,
    priceType: productData?.data?.data?.data?.priceType,
  };
  ...
  //update product there is no new image
  const updateProductMutationHandler = async (values:IProductFormProps) => {
    console.log(values)
    try {
      createProductMutation.mutate(
        {
          ...
          onSubmit={(val) => {
            ...
            if (val.image.length > 0) {
              ...
              setFormValues(val);
                ...
                updateProductMutationHandler(val);
            }
          }}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 w-full">
                ...
                <div className="w-full flex flex-col items-start space-y-1">
                  <Select
                    ...
                    defaultValue={
                      productData?.data?.data?.data?.priceType === "kg"
                        ? priceOptions[1]
                        : priceOptions[0]
                    }
                    isSearchable={false}
                ...
                <div className="w-full">
                  <Select
                    ...
                    className="w-full font-semibold"
                    options={category}
                    defaultValue={productData?.data?.data?.data?.category}
                    name="category"
                    isLoading={false}
